...
function Camera:setTarget(t)
  self.target = t
  if t then self.x, self.y = t.x, t.y end
end

function Camera:update(dt)
  if self.target then
    self.x = self.x + (self.target.x - self.x) * math.min(1, self.smooth * dt)
    self.y = self.y + (self.target.y - self.y) * math.min(1, self.smooth * dt)
  end
